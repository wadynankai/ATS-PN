...
// 一般的な方法です。この DLL 内のすべてのファイルは、コマンド ラインで定義された MAKEPATTERN_EXPORTS
// シンボルを使用してコンパイルされます。このシンボルは、この DLL を使用するプロジェクトでは定義できません。
// ソースファイルがこのファイルを含んでいる他のプロジェクトは、
// MAKEPATTERNERR_API 関数を DLL からインポートされたと見なすのに対し、この DLL は、このマクロで定義された
// シンボルをエクスポートされたと見なします。
#ifndef _ATSPLUGIN_H_INCLUDED_
#define _ATSPLUGIN_H_INCLUDED_

#ifdef MAKEPATTERNERR_EXPORTS
...
ATS_API void SetVehicleSpec(ATS_VEHICLESPEC);

// Called when the game is started
ATS_API void Initialize(int);

// Called every frame
ATS_API ATS_HANDLES Elapse(ATS_VEHICLESTATE, int*, int*);

// Called when the power is changed
ATS_API void SetPower(int);

// Called when the brake is changed
ATS_API void SetBrake(int);

// Called when the reverser is changed
ATS_API void SetReverser(int);

// Called when any ATS key is pressed
ATS_API void KeyDown(int);

// Called when any ATS key is released
ATS_API void KeyUp(int);

// Called when the horn is used
ATS_API void HornBlow(int);
...
ATS_API void SetSignal(int);

// Called when the beacon data is received
ATS_API void SetBeaconData(ATS_BEACONDATA);
